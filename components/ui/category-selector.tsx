...
const CategorySelectorComponent = ({ categories }: CategorySelectorProps) => {
    ...
    const onCommandItemSelect = (category: any) => {
    ...
    return (
        <Popover open={open} onOpenChange={setOpen}>
            ...
            <PopoverContent className="w-full p-0">
                <Command>
                    <CommandInput placeholder="Search category..." className="h-9" onKeyDown={commandInputKeyDown} />
                    <CommandList>
                        <CommandEmpty>No category found.</CommandEmpty>
                        <CommandGroup>
                            {categories.map((category) => (
                                <CommandItem key={category._id} value={category.title} onSelect={() => onCommandItemSelect(category)}>
                                    {category.title}
